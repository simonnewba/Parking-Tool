...
def gammln(xx):
    ...
    x = xx; y = xx; tmp = x + 5.5; tmp -= (x + 0.5) * np.log(tmp)
...
def gammp(a, x):
    """Regularized lower incomplete gamma function P(a,x). Pure-Python/numpy,
    no scipy dependency -- same algorithm validated in the original dashboard."""
    if x < 0 or a <= 0:
        return 0.0
    if x == 0:
        return 0.0
...
def run_one_simulation(garage_employee_counts, seed=None, horizon_min=125, dt_report=True,
    ...
    def pike_rate(t_sec, blocked):
        ...
        if phase_t < pike_green_sec:
            ...
            shape = 1.0 / (cfg.CV ** 2)
            scale = avg_gap * (cfg.CV ** 2)
            p_accept = 1 - gamma_cdf(cfg.EXTERNAL_CRITICAL_GAP_SEC, shape, scale)
